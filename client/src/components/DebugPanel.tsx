...
const DebugPanel: React.FC<DebugPanelProps> = ({ pointerInfo }) => {
  ...
  return (
    <div className="fixed top-[4rem] left-4 right-4 sm:left-auto sm:w-80 sm:right-4 max-h-[80vh] z-50">
      <ScrollArea className="h-full">
        <Card className="bg-background/95 backdrop-blur border-2 border-yellow-500">
          <Accordion type="single" collapsible defaultValue="debug-panel" className="w-full">
            <AccordionItem value="debug-panel">
              <CardHeader className="flex flex-row items-center w-full justify-between space-y-0 p-2">
                <AccordionTrigger className="flex-1 flex items-center justify-between py-0 hover:no-underline w-full">
              ...
              <AccordionContent className="pt-0">
                <CardContent className="space-y-4 text-xs p-4 pt-0">
                  {/* Editor State */}
                  <div>
